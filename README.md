...
1.  **Navigate to the project directory:**
    ```bash
    cd /home/ubuntu/MovieRater
    ```

2.  **Compile the Java code:**
    ```bash
    javac -cp lib/sqlite-jdbc.jar -d bin src/main/java/com/movierater/MovieRater.java
    ```

3.  **Run the application:**
    ```bash
    java -cp "bin:lib/sqlite-jdbc.jar" com.movierater.MovieRater
    ```

    *Note: On Windows, use `;` instead of `:` in the classpath: `java -cp "bin;lib/sqlite-jdbc.jar" com.movierater.MovieRater`*

## Usage

Upon running the application, a command-line menu will be displayed. Enter the number corresponding to the desired action and follow the prompts.

## TODOs for Completion

There are a few small gaps left for you to complete to ensure a full understanding of the project. These are marked with `// TODO` comments in the `MovieRater.java` file:

-   **`showMeanAge()` method**: Implement the SQL query to calculate the mean age of users.
-   **`showTotalUsersForMovie()` method**: Implement the SQL query to count unique users for a specific movie.

Complete these sections to finalize the application's functionality.
